    ...
    public static class Pathfinding
    {
        ...
        public static GoalEvaluator GoalInventoryHeuristic(string objectType, bool canTakeFromStockpile = true)
        {
            return current =>
            {
                ...
                if (current.Inventory != null && current.Inventory.objectType == objectType && current.Inventory.locked == false)
                {
                    // Type is correct and we are allowed to pick it up
                    if (canTakeFromStockpile || current.Furniture == null || current.Furniture.IsStockpile() == false)
                    {
        ...
        {
            return current => current.Furniture != null && current.Furniture.ObjectType == objectType;
        }
